...
try:
    ...
    app = FastAPI(
        ...
        description="Emergency Intelligence Platform",
        version="0.3.0",
    )
...
async def as_coroutine(value):
    """FIX: Wrap a plain value in a coroutine so asyncio.gather() accepts it."""
    return value


def _acled_has_credentials() -> bool:
    """
    FIX: Check credentials correctly.
    Original code used config.ACLED_API_KEY which doesn't exist —
    config uses ACLED_USERNAME / ACLED_PASSWORD instead.
    """
    try:
        return bool(
...
async def get_conflicts(
    ...
    if acled and available_collectors["ACLEDCollector"]:
        try:
            events = await acled.fetch_conflicts(country=country, days_back=days, min_fatalities=min_fatalities)
        ...
        try:
            ...
            for e in events:
                ...
                if loc not in hotspots:
                    hotspots[loc] = {"lat": e.get("latitude"), "lon": e.get("longitude"), "fatalities": 0, "events": 0, "location": e.get("location")}
...
async def get_flights_near(lat: float = 20.0, lon: float = 0.0, radius: float = 100, emergency_only: bool = False):
    if flights and available_collectors["FlightCollector"]:
        try:
            # FIX: renamed local variable to avoid shadowing the 'flights' collector
...
async def get_heatmap(disaster_type: Optional[str] = None, days: int = 30, points: int = 100):
    if heatmap and available_collectors["HeatmapCollector"]:
        try:
            return heatmap.generate_heatmap_data(disaster_type=disaster_type, days=days, points=points)
        except Exception as e:
...
async def get_forecast_heatmap():
    if views and available_collectors["VIEWSCollector"]:
        try:
            ...
            return [{"lat": f["lat"], "lon": f["lon"], "intensity": f["risk_score"] / 100, "color": views.get_risk_color(f["risk_score"])} for f in forecasts if f.get("lat") and f.get("lon")]
        except Exception as e:
            logger.error(f"Error: {e}")
...
async def get_dashboard_data(country: Optional[str] = None):
    """
    FIX: All tasks passed to asyncio.gather() are now proper coroutines.
    Previously, mock fallbacks returned plain lists — not awaitable — which
    caused gather() to fail silently or raise TypeErrors.
    We wrap them with as_coroutine() so gather() always receives awaitables.
    """
    try:
        tasks = []
        ...
        return {
            "timestamp": datetime.utcnow().isoformat(),
            "stats": {"total_conflicts": 3, "total_flights": 15, "emergency_flights": 2, "active_signals": 2, "military_bases": 5},
...
def get_mock_conflict_stats():
    return {"total_events": 150, "total_fatalities": 450, "by_type": {"Battles": 80, "Explosions": 40, "Violence against civilians": 30}, "by_severity": {"CRITICAL": 20, "HIGH": 45, "MODERATE": 50, "LOW": 35}}
...
def get_mock_heatmap():
    points = []
    base_points = [(18.5, -77.2, "earthquake"), (25.3, -86.5, "hurricane"), (14.5, 108.2, "flood"), (33.5, 36.3, "conflict"), (31.5, 34.5, "conflict"), (48.5, 37.5, "conflict")]
    for lat, lon, type_name in base_points:
        for _ in range(10):
            points.append({"lat": lat + random.uniform(-3, 3), "lon": lon + random.uniform(-3, 3), "intensity": random.uniform(0.5, 1.0), "type": type_name})
    return points


def get_mock_grid():
    return [{"lat": 18.5 + i * 0.5, "lon": -77.2 + j * 0.5, "intensity": random.uniform(0.3, 0.9)} for i in range(-2, 3) for j in range(-2, 3)]


def get_mock_timeline(hours=24):
    now = datetime.now()
    return [{"lat": 18.5 + random.uniform(-2, 2), "lon": -77.2 + random.uniform(-2, 2), "intensity": random.uniform(0.5, 1.0), "timestamp": (now - timedelta(hours=i)).isoformat(), "type": random.choice(["conflict", "earthquake", "flood"])} for i in range(hours) if random.random() > 0.7]


def get_mock_heatmap_stats():
    return {"total_points": 500, "by_type": {"conflict": 300, "earthquake": 100, "flood": 100}, "avg_intensity": 0.75, "max_intensity": 0.95, "min_intensity": 0.3, "active_zones": 12}
...
def get_mock_food_security():
    return [{"country": "Yemen", "phase": "IPC 5", "population": 16000000}, {"country": "South Sudan", "phase": "IPC 4", "population": 6500000}, {"country": "Haiti", "phase": "IPC 3", "population": 4500000}]
...
def get_mock_forecast_heatmap():
    return [{"lat": 33.5, "lon": 36.3, "intensity": 0.85, "color": "#8B0000"}, {"lat": 48.5, "lon": 37.5, "intensity": 0.75, "color": "#FF4444"}, {"lat": 17.5, "lon": -4.0, "intensity": 0.65, "color": "#FF8844"}, {"lat": 9.0, "lon": 38.7, "intensity": 0.55, "color": "#FF8844"}]
...
def get_mock_bases_near(lat, lon):
    return [{"name": "Nearby Base 1", "distance_km": 45, "country": "Unknown", "lat": lat + 0.5, "lon": lon + 0.5}, {"name": "Nearby Base 2", "distance_km": 120, "country": "Unknown", "lat": lat - 0.8, "lon": lon + 1.2}]


# FIX: Expanded mock helplines to cover all countries shown in the frontend dropdown
MOCK_HELPLINES = {
    "US": [
